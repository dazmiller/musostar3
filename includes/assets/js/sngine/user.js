...
function init_picture_position() {
    ...
    var position = $('.profile-cover-wrapper img').data('init-position');
...
$(function() {
                ...
                if(type == "photos") {
                    ...
                    if(handle == "cover-user" || handle == "cover-page" || handle == "cover-group" || handle == "cover-event") {
                        ...
                        if(cover_image.length == 0) {
                            $('.profile-cover-wrapper').prepend("<img style='top: 0px' data-init-position='0px' src='"+image_path+"' />")
                        } else {
    ...
    $('body').on('click', '.js_save-position-picture', function() {
        ...
        /* set cover image top as new val */
        var cover_image = $('.profile-cover-wrapper img');
        ...
        cover_image.data('init-position', position);
    ...
    $('body').on('click', '.js_cancel-position-picture', function() {
        ...
        $('.profile-cover-position-loader').hide();
        $('.profile-cover-position-buttons').hide();
        /* set cover image top position as init-position */
        var cover_image = $('.profile-cover-wrapper img');
        var position = cover_image.data('init-position');
        cover_image.css('top', position);
        /* destroy draggable cover image */
        cover_image.draggable("destroy");
    });
    ...
    $('body').on('click', '.js_verify, .js_unverify', function () {
        ...
        $.post(api['users/verify'], data , function(response) {
            if(response.callback) {
                ...
                if(_do == 'request') {
                    _this.after('<div class="btn btn-warning btn-delete js_unverify" '+html+'><i class="fa fa-clock mr5"></i>'+__["Pending"]+'</div>');
                } else {
    ...
    $('body').on('click', '.js_like-page, .js_unlike-page', function () {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'page-like') {
                    _this.replaceWith('<button type="button" class="btn btn-default js_unlike-page" data-id="'+id+'"><i class="fa fa-thumbs-up"></i> '+__['Unlike']+'</button>');
                } else {
                    _this.replaceWith('<button type="button" class="btn btn-primary js_like-page" data-id="'+id+'"><i class="fa fa-thumbs-up"></i> '+__['Like']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id, 'uid': uid} , function(response) {
            if(response.callback) {
                ...
                if(_this.hasClass('js_page-admin-addation')) {
                    _this.replaceWith('<button type="button" class="btn btn-default btn-delete js_page-admin-remove" data-id="'+id+'" data-uid="'+uid+'"><i class="fa fa-trash mr5"></i>'+__['Remove Admin']+'</button>');
                } else {
                    _this.replaceWith('<button type="button" class="btn btn-primary js_page-admin-addation" data-id="'+id+'" data-uid="'+uid+'"><i class="fa fa-check mr5"></i>'+__['Make Admin']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
            ...
            if(response.callback) {
                ...
                if(_this.hasClass('js_join-group')) {
                    if(privacy == "public") {
                        ...
                        _this.replaceWith('<button type="button" class="btn btn-default btn-delete js_leave-group" data-id="'+id+'" data-privacy="'+privacy+'"><i class="fa fa-clock"></i> '+__['Pending']+'</button>');
                    }
                } else {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id, 'uid': uid} , function(response) {
            if(response.callback) {
                ...
                if(_this.hasClass('js_group-admin-addation')) {
                    _this.replaceWith('<button type="button" class="btn btn-default btn-delete js_group-admin-remove" data-id="'+id+'" data-uid="'+uid+'"><i class="fa fa-trash mr5"></i>'+__['Remove Admin']+'</button>');
                } else {
                    _this.replaceWith('<button type="button" class="btn btn-primary js_group-admin-addation" data-id="'+id+'" data-uid="'+uid+'"><i class="fa fa-check mr5"></i>'+__['Make Admin']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'event-go') {
                    ...
                    _this.replaceWith('<button type="button" class="btn btn-success js_go-event" data-id="'+id+'"><i class="fa fa-calendar-check	"></i> '+__['Going']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
