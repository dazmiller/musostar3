...
function chat_box(user_id, conversation_id, name, name_list, multiple, link) {
    ...
    if($(chat_key).length == 0) {
        ...
        $(chat_key).find('.chat-widget-content').show();
        ...
        $.getJSON(api['messages/get'], data, function(response) {
            ...
            if(response.callback) {
                ...
                if(response.conversation_id) {
                    ...
                    $(chat_key).find('.x-form-tools-colors').show();
                }
                ...
                if(response.color) {
                    ...
                    $(chat_key).attr("data-color", response.color);
                    color_chat_box($(chat_key), response.color);
                }
            }
        })
        .fail(function() {
        ...
        if(!$(chat_key).hasClass('opened')) {
            $(chat_key).addClass('opened').find('.chat-widget-content').slideToggle(200);
        }
        $(chat_key).find('textarea').focus();
        /* reconstruct chat-widgets */
        reconstruct_chat_widgets();
        ...
        if(response.callback) {
            ...
            if(response.chat_boxes_opened) {
                $.each(response.chat_boxes_opened, function(i,conversation) {
                    chat_box(false, conversation.conversation_id, conversation.name, conversation.name_list, conversation.multiple_recipients);
                });
            }
            ...
            if(response.thread_updated) {
                console.log(response.thread_updated);
...
$(function() {
    ...
    // initialize chat (opened chatboxes)
    if(chat_enabled) {
        $.post(api['chat/initialize'], function(response) {
            ...
            if(response.callback) {
                ...
                $.each(response, function(i,conversation) {
                    chat_box(false, conversation.conversation_id, conversation.name, conversation.name_list, conversation.multiple_recipients, conversation.link);
                });
            }
        }, 'json');
    }
            ...
            if(response.callback) {
                ...
                if(widget.hasClass('fresh')) {
                    if(window.location.pathname.indexOf("messages") != -1) {
                        ...
                        widget.remove();
                        chat_box(false, response.conversation_id, response.name, response.name_list, response.multiple_recipients, response.link);
                    }
                } else {
                    if(conversation_id === undefined) {
    ...
    $('body').on('click', '.chat-widget-head', function() {
        ...
        if(widget.hasClass('js_chat-widget-master') && $('body').data('chat-enabled') != 1) {
            return;
        }
        /* toggle 'opened' class */
