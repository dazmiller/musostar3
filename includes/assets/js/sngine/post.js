...
$(function() {
    ...
    $('body').on('click', '.js_publisher-product', function() {
        ...
        var feeling = feeling_meta.find('input');
        _this.button('loading');
        $.post(api['posts/reaction'], {'do': 'add_product', 'product': JSON.stringify(product), 'message': textarea.val(), 'photos': JSON.stringify(photos), 'feeling_action':feeling.data('action'), 'feeling_value':feeling.val()}, function(response) {
            if(response.callback) {
                _this.button('reset');
                eval(response.callback);
            }
