...
try {
	...
	switch ($_GET['do']) {
		case 'edit':
			...
			/* update */
			$db->query(sprintf("UPDATE static_pages SET page_url = %s, page_title = %s, page_text = %s, page_in_footer = %s WHERE page_id = %s", secure($_POST['page_url']), secure($_POST['page_title']), secure($_POST['page_text']), secure($_POST['page_in_footer']), secure($_GET['id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			/* return */
		...
		case 'add':
			...
			$_POST['page_in_footer'] = (isset($_POST['page_in_footer']))? '1' : '0';
			/* insert */
			$db->query(sprintf("INSERT INTO static_pages (page_url, page_title, page_text, page_in_footer) VALUES (%s, %s, %s, %s)", secure($_POST['page_url']), secure($_POST['page_title']), secure($_POST['page_text']), secure($_POST['page_in_footer']) )) or _error(SQL_ERROR_THROWEN);
			/* return */
